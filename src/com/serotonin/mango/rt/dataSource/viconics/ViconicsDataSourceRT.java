...
public class ViconicsDataSourceRT extends EventDataSource implements ViconicsNetworkListener {
    ...
    public void initialize() {
        ...
        network.setPointValueMinimumFreshness(vo.getPointValueMinimumFreshnessSeconds() * 1000);
        ...
        // If we've made it this far, things should be pretty much ok. Deactivate any existing event.
    ...
    public void viconicsDeviceAdded(ViconicsDevice device) {
        ...
        synchronized (newDeviceLock) {
            ...
            String watchListName = tstat.getModel() + "_" + device.getCommAddress();
            ...
            for (int address : tstat.getAddresses()) {
                ...
                if (folderId == -1) {
                    ...
                    if (folder == null) {
                        ...
                        pointHierarchy.addPointFolder(folder, pointHierarchy.getRoot().getId());
                ...
                dp.setXid(tstat.getModel() + "_" + StatPoint.addressToString(address) + "_"
                        + System.currentTimeMillis());
                dp.setName(pointConfig.getName() + " (" + device.getCommAddress() + ")");
                ...
                if (pointConfig.isWatchlist()) {
                    if (watchlists == null) {
                        ...
                        for (User user : userDao.getActiveUsers()) {
                            ...
                            for (WatchList wl : watchListDao.getWatchLists(user.getId())) {
                            ...
                            if (watchList == null) {
                                // Didn't find an existing watchlist, so create one
                                watchList = new WatchList();
                                watchList.setName(watchListName);
                                watchListDao.createNewWatchList(watchList, user.getId());
                            }
    ...
    public void viconicsDeviceStatus(ViconicsDevice device, boolean online) {
        if (online)
            ...
            raiseEvent(DEVICE_OFFLINE_EVENT, System.currentTimeMillis(), true, new LocalizableMessage(
                    "event.viconics.deviceOffline", device.getIeeeString()));
        }
    }
    ...
    public void setPointValue(DataPointRT dataPoint, PointValueTime pvt, SetPointSource source) {
        ...
        if (vo.isConvertToCelsius()) {
            ViconicsDevice device = network.getDeviceWithIeee(locator.getDeviceIeee());
    ...
    public <T extends ViconicsIncomingResponse> T send(ViconicsOutgoingRequest request)
            throws ViconicsTransportException, RequestFailureException {
